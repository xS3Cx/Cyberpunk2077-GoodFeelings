...
local function formatBindingKeyboardOnly(b)
    if not b or not b.keys or #b.keys == 0 then
        return L("optionmanager.none")
    end
    local keyParts = {}
    for _,k in ipairs(b.keys) do
        table.insert(keyParts, Language.GetInputKey(k))
    end
    return table.concat(keyParts, " + ")
end

local function formatBindingControllerOnly(b)
    if not b or not b.btns or #b.btns == 0 then
        return L("optionmanager.none")
    end
    local btnParts = {}
    for _,bt in ipairs(b.btns) do
        table.insert(btnParts, Language.GetInputButton(bt))
    end
    return table.concat(btnParts, " + ")
end
...
---@param tip string|nil
---@return boolean clicked
function BindButton.OptionKeyboard(label, action, tip)
    local binding = Bindings.Actions[action] or {}
    local display = formatBindingKeyboardOnly(binding)  -- ONLY CHANGE: use keyboard-only format
    ...
    if rebinding.action == action then
        local elapsed = os.clock() - (rebinding.startTime or 0)
        if elapsed >= rebinding.countdown then
            if Input.IsKeyDown(Input.VK.ESCAPE) then
                Notification.Warning("Key binding cancelled. Restored original binding for "..label, 3)
                State.bindingKey = false
                rebinding.action, rebinding.heldKeys, rebinding.heldBtns, rebinding.startTime, rebinding.wasAnyDown = nil, {}, {}, nil, false
                return false
...
---@param tip string|nil
---@return boolean clicked
function BindButton.OptionController(label, action, tip)
    local binding = Bindings.Actions[action] or {}
    local display = formatBindingControllerOnly(binding)  -- ONLY CHANGE: use controller-only format
